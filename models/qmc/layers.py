...
class QFeatureMapScaler(tf.keras.layers.Layer):
    """Quantum feature map that just normalize each input vector.
    ...
    def call(self, inputs):
        ...
        psi = inputs / tf.expand_dims(norms, axis=-1)
        return psi
...
class QFeatureMapScaler_l1(tf.keras.layers.Layer):
    """Quantum feature map that just normalize each input vector using l1 norm.
    ...
    def call(self, inputs):
        norms = tf.linalg.norm(inputs, ord = 1, axis=1)
        psi = inputs / tf.expand_dims(norms, axis=-1)
        return psi
